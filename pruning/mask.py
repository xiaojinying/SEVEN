...
import pandas as pd
import torch
from .modules import LinearMasked
...
def mask_module(module, masks):
    ...
    for name, submodule in module.named_children():
        ...
        if submodule in masks:
            mask_kwargs = {'weight_mask': masks[submodule]}
    ...
    #         model.weight_mask=mask.to(torch.device('cuda:0'))
    #     else:
    #         mask_kwargs = {'weight_mask': mask}
    #         model=masked_modules[type(model)](model,**mask_kwargs).to(torch.device('cuda:0'))
